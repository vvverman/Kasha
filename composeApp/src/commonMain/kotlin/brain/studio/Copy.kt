...
loadFailed|Не удалось прочитать данные. Сохранённые данные не удалены. Проверьте доступ к хранилищу и повторите попытку.|Could not read data. Saved data was not deleted. Check storage access and retry.|No se pudieron leer los datos. Los datos guardados no se eliminaron. Comprueba el acceso al almacenamiento y reintenta.|Lecture des données impossible. Les données enregistrées sont conservées. Vérifiez l’accès au stockage et réessayez.|Daten konnten nicht gelesen werden. Gespeicherte Daten bleiben erhalten. Speicherzugriff prüfen und erneut versuchen.|Не вдалося прочитати дані. Збережені дані не видалено. Перевірте доступ до сховища й повторіть спробу.|Не ўдалося прачытаць даныя. Захаваныя даныя не выдалены. Праверце доступ да сховішча і паўтарыце спробу.|Деректерді оқу мүмкін болмады. Сақталған деректер жойылған жоқ. Қоймаға қолжетімділікті тексеріп, қайталап көріңіз.
